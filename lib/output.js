...
function addOutputs(command, outputs){
    if(!outputs || !outputs.length || outputs.length < 1) throw new Error('output(s) must be specified')
...
function addOutput(command, output){
    ...
    const localOutput = tmp.tmpNameSync()
    command.output(localOutput).format(output.format)
