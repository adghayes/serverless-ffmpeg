...
class PeakStore {
  ...
  update(buffers) {
    ...
    if (this.lastMax === undefined) {
      ...
      this.splitPeaks = Array(channels)
        .fill(null)
        .map((i) => []);
    }
...
class PeakFinder {
  ...
  onChunkRead(chunk) {
    let i = 0;
    let value;
    let samples = Array(this.numChannels)
      .fill(null)
      .map((i) => []);
...
function getPeaks(event) {
  if (!event.peaks.intermediary)
    throw new Error("server did not prepare file for finding peaks");
