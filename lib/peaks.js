...
function peaksInput(quality = 0.5){
	quality = Math.min(Math.max(quality, .1), 1)
    return {
        "format": "s16le",
		"options": `-ar ${Math.round(44100 * quality ** 3)}`,
		"audio": true
    }
}

module.exports = { Finder: PeaksFinder, input: peaksInput }
